...
export default async function AdminResourcesPage({ searchParams }: { searchParams: Promise<{ edit?: string }> }) {
  ...
  return <div><p className="kicker">Resources</p><h1>{current ? "Edit article" : "Learning article management"}</h1>
    <form action={saveResource} className="panel admin-form">
      ...
      <Field name="coverImage" label="Image URL" value={current?.coverImage} />
    ...
    <table className="table"><thead><tr><th>Title</th><th>Category</th><th>Status</th><th>Updated</th><th>Actions</th></tr></thead><tbody>{articles.map((article) => <tr key={article.id}><td>{article.title}</td><td>{article.category.name}</td><td>{article.status}</td><td>{article.updatedAt.toLocaleDateString()}</td><td className="table-actions"><a className="btn secondary compact" href={`/admin/resources?edit=${article.id}`}>Edit</a><form action={deleteResource}><input type="hidden" name="id" value={article.id} /><button className="btn danger compact">Delete</button></form></td></tr>)}</tbody></table>
  </div>;
}
function Field({ name, label, value }: { name: string; label: string; value?: string | null }) { return <label className="field"><span>{label}</span><input name={name} defaultValue={value ?? ""} required={!["coverImage", "seoTitle", "seoDescription"].includes(name)} /></label>; }
