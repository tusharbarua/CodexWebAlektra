...
export default async function AdminResourcesPage({ searchParams }: { searchParams: Promise<{ edit?: string; error?: string }> }) {
  ...
  return <div><p className="kicker">Resources</p><h1>{current ? "Edit article" : "Learning article management"}</h1>
    ...
    <table className="table"><thead><tr><th>Title</th><th>Category</th><th>Image</th><th>Status</th><th>Updated</th><th>Actions</th></tr></thead><tbody>{articles.map((article) => <tr key={article.id}><td>{article.title}</td><td>{article.category.name}</td><td>{article.coverImage ? "Uploaded" : "None"}</td><td>{article.status}</td><td>{article.updatedAt.toLocaleDateString()}</td><td className="table-actions"><a className="btn secondary compact" href={`/admin/resources?edit=${article.id}`}>Edit</a><form action={deleteResource}><input type="hidden" name="id" value={article.id} /><button className="btn danger compact">Delete</button></form></td></tr>)}</tbody></table>
  </div>;
}
