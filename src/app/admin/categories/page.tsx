...
function ResourceCategoryTable({ rows }: {
  rows: Array<{ id: string; name: string; slug: string; description?: string | null; _count: { articles?: number } }>;
}) {
  return <table className="table"><thead><tr><th>Name</th><th>Slug</th><th>Items</th><th>Actions</th></tr></thead><tbody>{rows.map((row) => {
    ...
    return <tr key={row.id}><td colSpan={2}><form action={saveResourceCategory} className="inline-form"><input type="hidden" name="id" value={row.id}/><input name="name" defaultValue={row.name} aria-label="Category name"/><input name="slug" defaultValue={row.slug} aria-label="Category slug"/><input type="hidden" name="description" value={row.description ?? "Category"}/><button className="btn secondary compact">Save</button></form></td><td>{count}</td><td><form action={deleteResourceCategory}><input type="hidden" name="id" value={row.id} /><button className="btn danger compact" disabled={count > 0}>Delete</button></form></td></tr>;
  })}</tbody></table>;
}
